...
Bulk mode  — checkbox list from batch sheet; process many clients at once.
Single mode — one client; fields auto-discovered from sheet column headers;
              existing row pre-filled if client already in that sheet.

All field definitions, formatters, and source types live in forms.json.
Adding a new form = JSON entry + Excel column headers. Zero code changes.
"""
import os
import sys
...
class FormFillerApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.btn_fill = tk.Button(
            bot, text="Fill & Save", width=18, height=2,
    ...
    def _build_bulk_panel(self, app: dict):
        ...
        try:
            batch = excel_reader.load_batch(self.xlsx_path, sheet, self.master_data)
        ...
        tk.Label(ctrl, text=f"{len(batch)} client(s) in '{sheet}' sheet",
        ...
        for client in batch:
            ...
            parts = [f"{k}: {client[k]}" for k in tx_cols[:4] if client.get(k)]
            label = f"  {client.get('name', '?'):<28}  " + "   |   ".join(parts)
            tk.Checkbutton(inner, text=label, variable=var,
                           anchor="w", font=("Courier", 9)).pack(fill=tk.X, padx=4, pady=1)
    ...
    def _build_single_panel(self, app: dict):
        ...
        ttk.Separator(self._panel, orient="horizontal").pack(fill=tk.X, pady=(6, 4))
        ...
        if not headers:
            ...
            tk.Label(inner,
                     text=f"Transaction fields — '{sheet}' sheet "
    ...
    def _autofill_single(self, sheet: str):
        ...
        if not ic:
            return
        existing = excel_reader.find_client_in_batch(self.xlsx_path, sheet, ic)
    ...
    def _run_fill(self, clients: list[dict], app: dict, label: str = ""):
        ...
        if all_warnings:
            parts.append(f"{len(all_warnings)} blank field warning(s).")
            messagebox.showwarning(
                "Blank fields detected",
                "These required fields were blank:\n\n" +
                "\n".join(all_warnings)
            )
